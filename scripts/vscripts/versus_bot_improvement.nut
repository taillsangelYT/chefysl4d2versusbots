...
function VBI_BotFireGrenade(bot, durationSec)
{
    ...
    bot.GetScriptScope()["grenade_fire_until"] <- Time() + durationSec;
}
...
function VBI_TickGrenadeFireButtons()
{
    ...
    for (local bot; bot = Entities.FindByClassname(bot, "player"); )
    {
        ...
        if ("grenade_fire_until" in scope)
        {
            ...
            else
            {
                ...
                // Switch back to primary/secondary after throw
                if (GetSecondarySlot(bot) != null)
                    bot.SwitchToItem(GetSecondarySlot(bot));
            }
        }
    }
}
...
function GrenadierBotsScript()
{
    ...
    for (local survbot; survbot = Entities.FindByClassname(survbot, "player"); )
    {
        ...
        {
            // Try to pick up a nearby grenade
            ...
            while (grenadenearby = Entities.FindByClassname(grenadenearby, "weapon_*"))
            {
                ...
                local dist = (survbot.GetOrigin() - grenadenearby.GetOrigin()).Length();
                if (dist > 150) continue;

                if (!("grenadier_bots_pick_timer" in scope)
                    || Time() < scope.grenadier_bots_pick_timer + 2) continue;
